...
from chemist import get_chem_data
    ...
    if request.method == "POST":                          #
        ...
        try :
            if client.verify(username,info_entered):      #Gives true if verified
                ...
                import algo
                u=algo.find_best_suggestion(medicine,username)
                import chemist
                final=get_chem_data(u)
#At this moment info_entered variable contains : [email,password,mobile,city,state] for client's account.
                st=[u,final["city"],final["state"],final["phone_num"],final["email"]]
#Create code such that this st list contains the best store's data in this order : 
# [username,city,state,mobile,email]
                numbers = [0,1,2,3,4]                       #
                give=["Shop name","City","State","Mobile","email"]#
